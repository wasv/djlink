...
static unsigned char *store_lidata(unsigned char *li) {
...
void write_output(char *filename) {
  ...
  for (int oi = 0; oi < objs.count; oi++) {
    ...
    for (int fi = 1; fi < o->segfrags.count; fi++) {
      ...
      if (f->combine == COMBINE_STACK && stack_addr < f->size + f->phys_addr) {
    ...
    for (int ri = 0; o->records[ri]; ri++) {
      ...
      case LEDATA:
        ...
        // printf("LEDATA at 0x%05x to 0x%05x\n", sf->phys_addr+frag_offset,
        // sf->phys_addr+frag_offset+rl-4);
