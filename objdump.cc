  ...
  while ((rectype = fgetc(f)) != EOF) {
    ...
    case 0x98:
      ...
      printf("\nSEGDEF:   [%d] = A=%d C=%d B=%d P=%d %04x [%d] [%d] [%d]\n",
             seg_count++, (buf[0] >> 5) & 7, (buf[0] >> 2) & 7,
             (buf[0] >> 1) & 1, buf[0] & 1, i, buf[3], buf[4], buf[5]);
      break;
    ...
    case 0x9c:
      ...
      while (i < reclen - 1) {
        if (buf[i] & 0x80) {
          /* fixup */
          printf("\tfix %s %03x; ", fixup_locations[(buf[i] >> 2) & 0x0f],
                 (buf[i] & 0x03) * 256 + buf[i + 1]);
          i += 2;
          int fix = buf[i++];
          if (fix & 0x80)
            printf(" F[%d]", (fix & 0x30) >> 4);
          else
            printf(" frame %s", frame_names[(fix & 0x70) >> 4]);
          if (fix & 0x08)
            printf(" T[%d]+%d", fix & 0x03, fix & 4);
          else
            printf(" target %s", target_names[fix & 0x07]);
          if ((fix & 0xc0) == 0)
            printf(" frame=%d", buf[i++]);
          if ((fix & 0x08) == 0)
            printf(" target=%x", buf[i++]);
          if ((fix & 0x04) == 0) {
            printf(" disp=%x", buf[i] + buf[i + 1] * 256);
            i += 2;
          }
          printf("\n");
        } else if (buf[i] & 0x40) {
          ...
          /* target */
          printf("\tTarget [%d] %s or %s", buf[i] & 3,
                 target_names[(buf[i] >> 2) & 3],
                 target_names[4 + ((buf[i] >> 2) & 3)]);
          i += 1;
          printf("\n");
        }
      }
