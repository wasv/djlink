...
static void print_symbols(FILE *map) {
  for (int yi = 0; yi < symtab.syms.count; yi++) {
    Symbol *y = (Symbol *)symtab.syms.list[yi];
    fprintf(map, " %04X:%04X  %s  %s\n", y->frame, y->offset,
            y->absolute ? "Abs" : "   ", y->name);
  }
}

static int sort_syms_byaddr(const void *va, const void *vb) {
  Symbol *a = *(Symbol **)va;
  Symbol *b = *(Symbol **)vb;
  int rv;
  rv = b->absolute - a->absolute;
  if (rv == 0)
    rv = a->phys_addr - b->phys_addr;
  if (rv == 0)
    rv = a->frame - b->frame;
  if (rv == 0)
    rv = stricmp(a->name, b->name);
  return rv;
}

static int sort_syms_byname(const void *va, const void *vb) {
  Symbol *a = *(Symbol **)va;
  Symbol *b = *(Symbol **)vb;
  return stricmp(a->name, b->name);
}

void print_map(char *mapfile) {
  ...
  for (int ci = 0; ci < segclasses.count; ci++) {
    ...
    for (int si = 0; si < c->parts.count; si++) {
      ...
      if (end < phys_addr)
        ...
        fprintf(map, " %05XH %05XH %05XH %-19s%s\n", phys_addr, end,
                s->size ? end - phys_addr + 1 : 0, s->name, s->segclass->name);
    }
  }
    ...
    for (int si = 0; si < c->parts.count; si++) {
      ...
      for (int fi = 0; fi < s->parts.count; fi++) {
        ...
        fprintf(map, "%04X:%04X %04X C=%s S=%s G=%s M=%s ACBP=%02x\n",
                ...
                f->phys_addr - f->segment->phys_addr, f->size,
                f->segment->segclass->name, f->segment->name,
                f->group->name[0] ? f->group->name : "(none)", f->obj->name,
                f->acbp);
      }
    }
  }
  ...
  qsort(symtab.syms.list, symtab.syms.count, sizeof(symtab.syms.list[0]),
        sort_syms_byname);
  print_symbols(map);
