...
void GSCF::dump() {
  if (name[0])
    printf("%05x %05x %04x%*c%s\n", phys_addr, size, alignment,
           print_depth * 4 - 3, ' ', name);
}
...
SegClass::SegClass() : GSCF() { print_depth = 2; }
  ...
  for (int ci = 0; ci < segclasses.count; ci++) {
    ...
    for (int si = 0; si < c->parts.count; si++) {
      ...
      for (int fi = 0; fi < c->parts.count; fi++) {
        ...
        if (f->combine == COMBINE_COMMON && s->alignment < f->alignment)
